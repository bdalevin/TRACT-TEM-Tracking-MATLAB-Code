function plot_jumps(series, jumps)
...
%   Written by Barnaby Levin, ASU, 2017
    ...
    scatter(ax2, jumps(:,1), jumps(:,2), [], jumps(:,3), 'LineWidth',1.5, 'SizeData', 49); 
    caxis ([0 9]);
...
%     b = num2str(jumps(:,5)); c = cellstr(b);
%     dx = 3; dy = 3;
%     text(jumps(:,1)+dx, jumps(:,2)+dy,c, 'Fontsize', 7, 'Color', 'w');
    ...
    Ctitle = ylabel(cb2, 'Jump Frequency (number per image series)'); % Give colorbar a title
