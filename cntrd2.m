function out=cntrd2(im,mx,sz,Thresh,interactive)
...
ind=find(mx(:,2) > 1*sz & mx(:,2) < nr-1*sz);
mx=mx(ind,:);
ind=find(mx(:,1) > 1.2*sz & mx(:,1) < nc-1.2*sz);
mx=mx(ind,:);
...
for i=1:nmx
    ...
    val = max(max(tmp)); 
    [row,col] = find(tmp>val*Thresh);
    sizer=size(row);
    intensity = 0;
    for j = 1:sizer(1)
        intensity = intensity + tmp(col(j), row(j));
    end

    
    %concatenate it up
    pts=[pts,[mx(i,1)+xavg-r,mx(i,2)+yavg-r,intensity,norm,rg,ID(i)]'];
