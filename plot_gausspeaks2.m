function plot_gausspeaks2(series, Gaussian, xtarget, ytarget, window)
...
% 
    ...
    %Display first image
    imagesc( series(:,:,imnum) ); axis image; 
    hold on; % Hold on forces every subsequent plot to be plotted on top of the image
    scatter(Gaussian(:,2,imnum),Gaussian(:,4,imnum),'x','c');  % Scatter plot the peaks found for the first image on top of the fist image
    brush on;
    xlim([xtarget-window xtarget+window]); % Ensure x axes of scatter plot and image are the same
    ylim([ytarget-window ytarget+window]); % Ensure y axes of scatter plot and image are the same
    ...
    key = 'default';
        while( key ~= 'q' )
            ...
            if key == 'p'
                ...
                xlim([xtarget-window xtarget+window]); % Ensure x axes of scatter plot and image are the same
                ylim([ytarget-window ytarget+window]); % Ensure y axes of scatter plot and image are the same
                scatter(Gaussian(:,2,imnum),Gaussian(:,4,imnum),'x','c');
                brush on;
                xlim([xtarget-window xtarget+window]); % Ensure x axes of scatter plot and image are the same
                ylim([ytarget-window ytarget+window]); % Ensure y axes of scatter plot and image are the same         
                hold off;
