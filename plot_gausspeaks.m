function plot_gausspeaks(series, Gaussian, xtarget, ytarget, window)
...
% 
    ...
    scatter(Gaussian(:,2,imnum),Gaussian(:,4,imnum),500,'x','c', 'LineWidth',2);  % Scatter plot the peaks found for the first image on top of the fist image
    brush on;
    ...
    key = 'default';
        while( key ~= 'q' )
            ...
            if key == 'p'
                ...
                scatter(Gaussian(:,2,imnum),Gaussian(:,4,imnum),500,'x','b', 'LineWidth',2);
                brush on;
            ...
            elseif key == 'o'
                ...
                imnum = imnum - ~limreached;
                axis image;  axis off; colormap('gray'); set(gcf, 'color', [1 1 1]); hold on;
                scatter(Gaussian(:,2,imnum),Gaussian(:,4,imnum),500,'x','r', 'LineWidth',2);
                brush on;
